...
sealed trait Tree[A]
case class Item[A](value :A) extends Tree[A]
case class Section[A] (subtree:List[A]) extends Tree[A]

sealed trait Path[A]
case object Top extends Path[Nothing]
case class Node[A](left:List[Tree[A]], parent:Path[A], right:List[Tree[A]]) extends Path[A]

case class Zipper[A](focus : A, path : Path[A])

def goDown( start:Zipper[A]): Either[TraversalError, Zipper[A]] = 
  start.focus match {
    case Item(item) => Left(Down)
  }
